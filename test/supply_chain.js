...
contract('SupplyChain', async accounts => {
  it("should create a Participant", async () => {
    ...
    let participantId = await instance.addParticipant("A","passA","0x6a049dF1eD7F83B0668575748290b19c19540566","Manufacturer");
    let participant = await instance.participants(0);
    assert.equal(participant[0], "A");
    assert.equal(participant[2], "Manufacturer");

    participantId = await instance.addParticipant("B","passB","0xF4e2f322b955f2378F9852E38313e393A6a61359","Supplier");
    participant = await instance.participants(1);
    assert.equal(participant[0], "B");
    assert.equal(participant[2], "Supplier");

    participantId = await instance.addParticipant("C","passC","0x1Eb43EBde750C57AaCEE85777e7c063218B0e1B4","Consumer");
